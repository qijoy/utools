...
export class SealGenerator {
  ...
  public async generate(outputPath: string): Promise<string> {
    ...
    // 做旧效果
    if (aging) {
      this.addAgingEffect(ctx, type === "circle" ? size : width, agingStrength);
    }
  ...
  private drawRectSeal(
    ...
    if (vertical) {
      // 竖排优化：增大字间距，整体居中
      const chars = text.split("");
      const spacing = fontSize * 1.3; // 字间距更大
      const totalHeight = chars.length * spacing;
      chars.forEach((char, i) => {
        ctx.fillText(
          char,
          width / 2,
          (height - totalHeight) / 2 + i * spacing + spacing / 2
        );
      });
    } else {
      // 横排
